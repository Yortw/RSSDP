...
namespace Rssdp
{
  ...
  public sealed class DiscoveredSsdpDevice
  {
    ...
    public async Task<SsdpRootDevice> GetDeviceInfo(HttpClient downloadHttpClient)
    {
      ...
      {
        throw new ArgumentNullException(nameof(downloadHttpClient));
      }
      ...
      if (_Device == null || this.IsExpired())
      {
        ...
        _Device = new SsdpRootDevice(this.DescriptionLocation, this.CacheLifetime, Encoding.UTF8.GetString(data, 0, data.Length));
      }
