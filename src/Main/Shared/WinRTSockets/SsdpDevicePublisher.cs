...
namespace Rssdp
{
	...
	public class SsdpDevicePublisher : SsdpDevicePublisherBase
	{
		...
		/// </summary>
		/// <remarks>
		/// <para>Uses the default <see cref="ISsdpCommunicationsServer"/> implementation and network settings for Windows Phone (Silverlight) and the SSDP specification.</para>
		/// </remarks>
		...
		/// <remarks>
		/// <para>Uses the default <see cref="ISsdpCommunicationsServer"/> implementation and network settings for Windows Phone (Silverlight) and the SSDP specification, but specifies the local port to use for socket communications. Specify 0 to indicate the system should choose it's own port.</para>
		/// </remarks>3
		[System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope", Justification = "No way to do this here, and we don't want to dispose it except in the (rare) case of an exception anyway.")]
		...
		/// <remarks>
		/// <para>Uses the default <see cref="ISsdpCommunicationsServer"/> implementation and network settings for Windows Phone (Silverlight) and the SSDP specification, but specifies the local port to use and multicast time to live setting for socket communications.</para>
